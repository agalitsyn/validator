...
func TestValidator_AddFieldError(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantResult:  false,
			wantMessage: []string{"email is required"},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, tt.wantMessage, v.FieldErrors[tt.args.key])
		})
	}
}
...
func TestValidator_CheckField(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantResult: false,
			wantMsgs:   []string{"email is required"},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, tt.wantResult, v.Valid())
			assert.Equal(t, tt.wantMsgs, v.FieldErrors[tt.args.key])
		})
	}
}
